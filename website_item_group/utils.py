...
from erpnext.portal.product_configurator.utils import get_product_settings,get_item_codes_by_attributes,get_items,get_product_settings,get_attribute_filter_data

def get_field_filter_data():
	product_settings = get_product_settings()
	filter_fields = [row.fieldname for row in product_settings.filter_fields]
	# print(filter_fields,'filter_fields')
	print('---------------------------------------------------------[[[[[[[[[[[[[[',product_settings.enable_website_item_group)
	if product_settings.enable_website_item_group==1:
		filter_fields.append('website_item_groups')
	meta = frappe.get_meta('Item')
	fields = [df for df in meta.fields if df.fieldname in filter_fields]
	for df in meta.fields :
		if df.fieldname in filter_fields:
			print(df.fieldname,df.name,'=====')
	...
	for f in fields:
		...
		import json
		# print(str(f))
		if f.fieldname=='website_item_groups':
			doctype=f.options
		else:
		...
		if f.fieldname=='website_item_groups':
			values = [d.item_group for d in frappe.get_all(doctype,fields=['item_group'],filters=filters,group_by='item_group')]
			print(doctype,'values',len(values))
		else:
...
def get_items_by_fields(field_filters):
	...
	#  field_filters {'website_item_groups': ['Power Unit - Spreader -- Spare Parts Lists']}
	meta = frappe.get_meta('Item')
	filters = []
...
def get_products_html_for_website(field_filters=None, attribute_filters=None):
	print('get_products_html_for_website -get_products_html_for_website--------------------------------------------------------------')
...
def get_website_item_group_data():
	# filter_data = get_field_filter_data()
	filter_data=[]
	product_settings = get_product_settings()
	filter_fields = ['website_item_groups']
	...
	for f in fields:
		...
		if meta.has_field('disabled'):
			filters['disabled'] = 0
		for d in frappe.get_all(doctype, fields=['name', 'item_group'],filters=filters):
			print(d.name,d.item_group,'----')
		values = [d.item_group for d in frappe.get_all(doctype,fields=['name', 'item_group'],filters=filters)]
		filter_data.append([f, values])
		# print('v----------',values)
	print('---------filter_data---------',filter_data)
	return filter_data
